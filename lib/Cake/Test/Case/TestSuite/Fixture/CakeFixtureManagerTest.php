...
class CakeFixtureManagerTest extends CakeTestCase {
    ...
    public function testLoadTruncatesTable() {
        $MockFixture = $this->getMock ( 'UuidFixture', array (
            'truncate' 
        ) );
        $MockFixture->expects ( $this->once () )->method ( 'truncate' )->will ( $this->returnValue ( true ) );
        ...
        $loadedProperty->setValue ( $fixtureManager, array (
            'core.uuid' => $MockFixture 
        ) );
        
        $TestCase = $this->getMock ( 'CakeTestCase' );
        $TestCase->fixtures = array (
            'core.uuid' 
        );
    ...
    public function testLoadSingleTruncatesTable() {
        ...
        $fixtureMapProperty->setValue ( $fixtureManager, array (
            'UuidFixture' => $MockFixture 
        ) );
        
        $dboMethods = array_diff ( get_class_methods ( 'DboSource' ), array (
            'enabled' 
        ) );
        $dboMethods [] = 'connect';
