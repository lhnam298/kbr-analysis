...
class TestAuthentication {
     ...
     *            A HTTP socket.
     * @param
     *            array &$proxyInfo Some proxy info.
     * @return void
     */
...
class HttpSocketTest extends CakeTestCase {
    ...
    public function setUp() {
        parent::setUp ();
        $this->Socket = $this->getMock ( 'TestHttpSocket', array (
            'read',
            'write',
            'connect' 
        ) );
        $this->RequestSocket = $this->getMock ( 'TestHttpSocket', array (
            'read',
            'write',
            'connect',
            'request' 
    ...
    public function testRequest2() {
        ...
        $this->Socket->expects ( $this->any () )->method ( 'read' )->will ( $this->onConsecutiveCalls ( $serverResponse, false ) );
    ...
    public function testRequestWithConstructor() {
        ...
        $http = $this->getMock ( 'TestHttpSocket', array (
            'read',
            'write',
            'connect',
            'request' 
    ...
    public function testRequestWithResource() {
        ...
        $this->Socket->expects ( $this->any () )->method ( 'read' )->will ( $this->onConsecutiveCalls ( $serverResponse, false, $serverResponse, false ) );
        $this->Socket->connected = true;
    ...
    public function testRequestWithRedirectUrlEncoded() {
        ...
        $this->Socket->expects ( $this->at ( 3 ) )->method ( 'write' )->with ( $this->logicalAnd ( $this->stringContains ( 'Host: i.cmpnet.com' ), $this->stringContains ( 'GET /techonline/pdf/a+b.pdf' ) ) );
    ...
    public function testRequestWithRedirectAsTrue() {
        ...
        $this->Socket->expects ( $this->any () )->method ( 'read' )->will ( $this->onConsecutiveCalls ( $serverResponse1, false, $serverResponse2, false ) );
    ...
    public function testHead() {
        ...
        $this->RequestSocket->head ( 'http://www.google.com/', array (
            'foo' => 'bar' 
        ) );
        $this->RequestSocket->head ( 'http://www.google.com/', 'foo=bar' );
        $this->RequestSocket->head ( 'http://www.google.com/?foo=bar', array (
            'foobar' => '42',
            'foo' => '23' 
        ) );
        $this->RequestSocket->head ( 'http://www.google.com/', null, array (
            'version' => '1.0' 
        ) );
        $this->RequestSocket->head ( 'https://secure.example.com/test.php', array (
            'one' => 'two' 
        ) );
    ...
    public function testAuth() {
        ...
        $this->assertEquals ( $this->Socket->request ['auth'], array (
            'Basic' => array (
                'user' => 'joel',
                'pass' => 'hunter2' 
            ) 
