...
class CakeTimeTest extends CakeTestCase {
    ...
    public function testTimeAgoInWordsCustomStrings() {
        ...
        $result = $this->Time->timeAgoInWords ( strtotime ( '+8 years +4 months +2 weeks +3 days' ), array (
            'relativeStringFuture' => 'not in the next %s',
            'accuracy' => array (
                'year' => 'year' 
            ),
            'end' => '+10 years' 
    ...
    public function testTimeAgoInWordsWithFormat() {
        ...
        $result = $this->Time->timeAgoInWords ( strtotime ( '+2 weeks +2 days' ), '%x' );
        $this->assertRegExp ( '/^in 2 weeks, [1|2] day(s)?$/', $result );
    ...
    public function testListTimezones() {
        $this->skipIf ( version_compare ( PHP_VERSION, '5.4.0', '<=' ), 'This test requires newer libicu which is in php5.4+' );
