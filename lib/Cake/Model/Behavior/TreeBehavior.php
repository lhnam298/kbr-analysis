...
class TreeBehavior extends ModelBehavior {
    ...
    protected function _setChildrenLevel(Model $Model, $id) {
        ...
        $depths = array (
            $id => ( int ) $Model->data [$Model->alias] [$settings ['level']] 
        );
        
        $children = $Model->children ( $id, false, array (
            $primaryKey,
            $settings ['parent'],
            $settings ['level'] 
        ), $settings ['left'], null, 1, - 1 );
    ...
    public function children(Model $Model, $id = null, $direct = false, $fields = null, $order = null, $limit = null, $page = 1, $recursive = null) {
        ...
        $options = array_merge ( compact ( 'conditions', 'fields', 'order', 'limit', 'page', 'recursive' ), $options );
    ...
    protected function _getOptions($arg) {
        return count ( array_filter ( array_keys ( $arg ), 'is_string' ) ) > 0 ? $arg : array ();
    }
     ...
     * @param string $mode
     *            parent or tree
     * @param string|int|null $missingParentAction
