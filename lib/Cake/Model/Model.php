...
class Model extends CakeObject implements CakeEventListener {
     ...
     * @throws Exception
     * @throws PDOException @triggers Model.beforeSave $this, array($options)
    ...
    protected function _doSave($data = null, $options = array()) {
        ...
        foreach ( $dateFields as $updateCol ) {
            ...
            $fieldInWhitelist = (count ( $this->whitelist ) === 0 || in_array ( $updateCol, $this->whitelist ));
    ...
    public function saveMany($data = null, $options = array()) {
        ...
        try {
            ...
            foreach ( $data as $key => $record ) {
                ...
                if ($validates) {
                    if ($options ['deep']) {
                        $saved = $this->saveAssociated ( $record, array (
                            'atomic' => false 
                        ) + $options );
                    } else {
                        $saved = ( bool ) $this->save ( $record, array (
                            'atomic' => false 
                        ) + $options );
                    }
                }
     ...
     *
     * @param
     *            array &$data Record data to validate. This should be a numerically-indexed array
     * @param array $options
    ...
    public function saveAssociated($data = null, $options = array()) {
        ...
        try {
            ...
            foreach ( $data as $association => $values ) {
                ...
                if ($validates) {
                    if ($options ['deep']) {
                        $saved = $Model->saveAssociated ( $values, array (
                            'atomic' => false 
                        ) + $options );
                    } else {
                        $saved = ( bool ) $Model->save ( $values, array (
                            'atomic' => false 
                        ) + $options );
                    }
            ...
            if ($validates && ! ($this->create ( null ) !== null && $this->save ( $data, array (
                'atomic' => false 
            ) + $options ))) {
            ...
            foreach ( $data as $association => $values ) {
                ...
                switch ($type) {
                    ...
                    case 'hasMany' :
                        ...
                        $options = $Model->_addToWhiteList ( $key, $options );
                        $_return = $Model->saveMany ( $values, array (
                            'atomic' => false 
                        ) + $options );
     ...
     *
     * @param
     *            array &$data Record data to validate. This should be an array indexed by association name.
     * @param array $options
     *            Options to use when validating record data (see above), See also $options of validates().
    ...
    public function isUnique($fields, $or = true) {
        if (is_array ( $or )) {
            $isRule = (array_key_exists ( 'rule', $or ) && array_key_exists ( 'required', $or ) && array_key_exists ( 'message', $or ));
            if (! $isRule) {
                $args = func_get_args ();
                $fields = $args [1];
