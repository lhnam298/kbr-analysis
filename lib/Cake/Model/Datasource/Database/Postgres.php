...
class Postgres extends DboSource {
    ...
    public function connect() {
        ...
        try {
            $this->_connection = new PDO ( "pgsql:host={$config['host']};port={$config['port']};dbname={$config['database']};sslmode={$config['sslmode']}", $config ['login'], $config ['password'], $flags );
            ...
            foreach ( $cols as $c ) {
                ...
                if ($model instanceof Model) {
                    if ($c->name === $model->primaryKey) {
                        ...
                        if ($fields [$c->name] ['type'] !== 'string' && $fields [$c->name] ['type'] !== 'uuid') {
                            $fields [$c->name] ['length'] = 11;
                        }
                    }
                }
                if ($fields [$c->name] ['default'] === 'NULL' || $c->default === null || preg_match ( '/nextval\([\'"]?([\w.]+)/', $c->default, $seq )) {
                                ...
                                if ($boolToInt) {
                                    ...
                                    if ($original ['type'] === 'text' && $col ['type'] === 'integer') {
                                        $colList [] = 'ALTER COLUMN ' . $fieldName . ' TYPE ' . str_replace ( array (
                                            $fieldName,
                                            'NOT NULL' 
                                        ), '', $this->buildColumn ( $col ) ) . " USING cast({$fieldName} as INTEGER)";
                                    } else {
                                        $colList [] = 'ALTER COLUMN ' . $fieldName . ' TYPE ' . str_replace ( array (
                                            $fieldName,
                                            'NOT NULL' 
