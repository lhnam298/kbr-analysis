...
class DboSource extends DataSource {
     ...
     * Cached results for both DboSource::name() and DboSource::fields()
     * will be stored here.
     *
     ...
     *
     * @param
     *            array &$result Reference to the fetched row
     * @return void
     */
    ...
    /**
     * Filters to apply to the results of `name` and `fields`.
    ...
    public function queryAssociation(Model $Model, Model $LinkModel, $type, $association, $assocData, &$queryData, $external, &$resultSet, $recursive, $stack) {
        ...
        if ($type === 'hasMany' && empty ( $assocData ['limit'] ) && ! empty ( $assocData ['foreignKey'] )) {
            ...
            // Filter
            if ($queryData ['callbacks'] === true || $queryData ['callbacks'] === 'after') {
                $this->_filterResultsInclusive ( $assocResultSet, $Model, array (
                    $association 
                ) );
            }
            ...
            $joinKeys = array (
                $foreignKey,
                $habtmAssocData ['associationForeignKey'] 
            );
            ...
            if (isset ( $merge [$association] )) {
                ...
                if (! isset ( $dataAssociation )) {
                    ...
                    if (is_array ( $merge [0] [$association] )) {
                        ...
                        $dataAssociation = array_merge ( $dataAssociation, array (
                            $association => array () 
                        ) );
                    }
                }
     ...
     * @param bool $external
     *            Whether or not the association query is on an external datasource.
     * @return mixed String representing a query.
    ...
    public function getConstraint($type, Model $Model, Model $LinkModel, $association, $assocData, $association2 = null) {
        ...
        switch ($type) {
            ...
            case 'hasMany' :
                return array (
                    "{$association}.{$assocData['foreignKey']}" => array (
                        '{$__cakeID__$}' 
                    ) 
                );
            case 'hasAndBelongsToMany' :
    ...
    /**
     * Builds and generates a JOIN condition from an array.
    ...
    protected function _getJoins(Model $Model) {
        ...
        foreach ( $joins as $assoc ) {
            ...
            $join [] = $this->buildJoinStatement ( array (
                ...
                'alias' => $assoc,
                'type' => isset ( $assocData ['type'] ) ? $assocData ['type'] : 'LEFT',
                'conditions' => trim ( $this->conditions ( $this->_mergeConditions ( $assocData ['conditions'], $this->getConstraint ( $assocData ['association'], $Model, $LinkModel, $assoc, $assocData ) ), true, false, $Model ) ) 
            ) );
        }
     ...
     * @param string|array|bool $conditions
    ...
    protected function _quoteFields($conditions) {
        ...
        $conditions = preg_replace ( '/(\s[a-z0-9\\-_.' . $start . $end . ']*' . $end . ')\s+AS\s+([a-z0-9\\-_]+)/i', '\1 AS ' . $this->startQuote . '\2' . $this->endQuote, $conditions );
        if ($conditions !== null) {
            return $conditions;
        }
        return $original;
