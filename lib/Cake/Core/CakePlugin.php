...
class CakePlugin {
        ...
        if (empty ( $config ['path'] )) {
            foreach ( App::path ( 'plugins' ) as $path ) {
                if (is_dir ( $path . $plugin )) {
                    static::$_plugins [$plugin] = $config + array (
                        'path' => $path . $plugin . DS 
                    );
                    break;
                }
    ...
    public static function bootstrap($plugin) {
        ...
        if ($config ['bootstrap'] === true) {
            return static::_includeFile ( $path . 'Config' . DS . 'bootstrap.php', $config ['ignoreMissing'] );
        }
        
        $bootstrap = ( array ) $config ['bootstrap'];
        foreach ( $bootstrap as $file ) {
            static::_includeFile ( $path . 'Config' . DS . $file . '.php', $config ['ignoreMissing'] );
        }
    ...
    public static function routes($plugin = null) {
        ...
        return ( bool ) static::_includeFile ( static::path ( $plugin ) . 'Config' . DS . 'routes.php', $config ['ignoreMissing'] );
    }
