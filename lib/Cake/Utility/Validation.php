...
class Validation {
     ...
     *            'all' may be passed as a sting, defaults to fast which checks format of most major credit
     *            cards
    ...
    public static function url($check, $strict = false) {
        ...
        $regex = '/^(?:(?:https?|ftps?|sftp|file|news|gopher):\/\/)' . (! empty ( $strict ) ? '' : '?') . '(?:' . static::$_pattern ['IPv4'] . '|\[' . static::$_pattern ['IPv6'] . '\]|' . static::$_pattern ['hostname'] . ')(?::[1-9][0-9]{0,4})?' . '(?:\/?|\/' . $validChars . '*)?' . '(?:\?' . $validChars . '*)?' . '(?:#' . $validChars . '*)?$/iu';
        return static::_check ( $check, $regex );
    }
    ...
    public static function uploadError($check, $allowNoFile = false) {
        ...
        if ($allowNoFile) {
            return in_array ( ( int ) $check, array (
                UPLOAD_ERR_OK,
                UPLOAD_ERR_NO_FILE 
            ), true );
        }
