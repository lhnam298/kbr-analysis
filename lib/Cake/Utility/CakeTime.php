...
class CakeTime {
        ...
        switch ($specifier [1]) {
            ...
            case 'r' :
                ...
                if ($complete !== 't_fmt_ampm') {
                    return str_replace ( '%p', static::_translateSpecifier ( array (
                        '%p',
                        'p' 
                    ) ), $complete );
                }
     ...
     *
     * @param int|string|DateTime $dateString
     ...
     *
     * @param int|string|DateTime $begin
     *            UNIX timestamp, strtotime() valid string or DateTime object
    ...
    public static function gmt($dateString = null) {
        ...
        return gmmktime ( ( int ) date ( 'G', $time ), ( int ) date ( 'i', $time ), ( int ) date ( 's', $time ), ( int ) date ( 'n', $time ), ( int ) date ( 'j', $time ), ( int ) date ( 'Y', $time ) );
    }
     ...
     * @param int|string|DateTime $format
        ...
        if ($group) {
            ...
            foreach ( $identifiers as $key => $tz ) {
                ...
                if ($options ['abbr']) {
                    ...
                    $abbr = isset ( $trans [0] ['abbr'] ) ? $before . $trans [0] ['abbr'] . $after : null;
                }
                $item = explode ( '/', $tz, 2 );
                if (isset ( $item [1] )) {
